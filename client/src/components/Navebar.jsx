...
const NavebarItems = ({title, classProps}) =>{
    return(
...
const Navebar = ()=>{
    ...
    return (
        <div className='w-full flex md:justify-center justify-between items-center p-4'>
            <div className='md:flex-[0.5] f lex-initial justify-center items-center'>
                <img src={logo} alt="logo" className='w-32 cursor-pointer' />
            </div>
            <ul className='text-white md:flex hidden list-none flex-row justify-between items-center flex-initial'>
                {["Market", "Exchange", "Tutorial", "Wallets"].map((item, index)=>(
                    <NavebarItems key={item + index} title={item}/>                
                ))}
            ...
            <div className='flex relative'>
                ...
                {toggleMenu && (
                    ...
                    >
                        ...
                        {["Market", "Exchange", "Tutorial", "Wallets"].map((item, index)=>(
                            <NavebarItems key={item + index} title={item} classProps="my-2 text-lg"/>                
                        ))}
                    </ul>
                )}
            </div>
